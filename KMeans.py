...
class KMeans():
    ...
    def fit(self, input: np.ndarray) -> np.array: 
        ...
        for i in range(self.max_iter): # iterate until it reaches max iterations
            ...
            if (np.array_equal(previous, self.cluster_centers_)):  # if no centroid movement return    
                ...
                previous = self.cluster_centers_
    ...
    def calculate_distance(self, d_features, c_features) -> int:
        ...
        """
        return math.sqrt(math.pow(c_features[0] - d_features[0], 2) + 
                         math.pow(c_features[1] - d_features[1], 2) + 
                         math.pow(c_features[2] - d_features[2], 2) + 
                         math.pow(c_features[3] - d_features[3], 2) + 
                         math.pow(c_features[4] - d_features[4], 2))
